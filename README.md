...
running *Locator* on a CUDA-enabled GPU (Installation 
...
You can train replicate models on bootstrap samples of the full VCF (sampling SNPs with replacement) with the 
...
```
This is slow (you're fitting new models to each replicate), but should give a good idea of uncertainty in predicted locationss. A quicker and probably worse estimate of uncertainty can also be generated by the `--jacknife` option. This uses a single trained model and generates predictions while treating 5% of sites as missing data. We're not sure it's a good idea yet, but it's fast and sometimes helps when predictions are split between regions. You can run that with:
...
# Plotting
We include an R script to plot output from Locator. Install the required packages by running 
...
```
This will plot predictions and uncertainties for 9 randomly selected individuals to `/out/jacknife/test_windows.png.` For lat/long coordinates
you can also calculate and plot error estimates by using the `--error` option. See all parameters with 
```
