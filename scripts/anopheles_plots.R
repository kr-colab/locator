...
xcoords <- pred[,seq(1,ncol(pred)-1,2)]
meanx <- rowMeans(xcoords)
ycoords <- pred[,seq(2,ncol(pred)-1,2)]
meany <- rowMeans(ycoords)
pred <- data.frame(meanx,meany,pred[,ncol(pred)])
...
p <- ggplot()+coord_map(xlim = c(min(pd$longitude.y)-5,max(pd$longitude.y)+5),
  ...
  theme(axis.line=element_blank(),
        ...
        legend.box.margin = margin(-20,0,0,40),
  ...
  geom_point(data=pd,aes(x=longitude.x,y=latitude.x,fill=species),size=1.5,shape=21,stroke=0.01)+
